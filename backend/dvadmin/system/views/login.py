...
class SignInView(APIView):
    ...
    def post(self,request):
        ...
        try:
            user= Users.objects.create_user(username=username,password=password,user_type=1)
            return Response({"msg":"注册成功","code":2000,"data":{"username":username,"userId":user.id,"user_type":user.user_type}})
        except Exception as e:
...
class UserSerializer(CustomModelSerializer):
    ...
    class Meta:
        ...
        fields = "__all__"
        read_only_fields = ["id"]
        ref_name = "GetUserSerializer"

class GetUserFilter(django_filters.FilterSet):
    username = django_filters.CharFilter(field_name="username", lookup_expr="icontains", label="用户名")
    mobile = django_filters.CharFilter(field_name="mobile", lookup_expr="icontains", label="手机号")
    class Meta:
        model = Users
        fields = ['username', 'mobile']

class GetUserListView(CustomModelViewSet):
    """
    获取移动端用户列表
    """
    permission_classes = [IsAuthenticated]
    serializer_class = UserSerializer
    filter_class = GetUserFilter
    queryset = Users.objects.filter(user_type=1).order_by('-id')
